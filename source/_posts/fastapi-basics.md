...
以前觉得"写后端接口"是件特别神秘的事，直到上手 FastAPI，才发现原来就是给 Python 函数贴个标签，然后它自己就变成能对外服务的接口了。这篇把我学到的东西整理一遍，纯大白话，保证看完能"依葫芦画瓢"跑起来。
...
一句话总结：**给函数贴标签 → 声明参数类型 → 让 FastAPI 自动校验和生成文档**，剩下的脏活它全包了。

三个实用建议：

1. **类型一定要标**：`userid: int` 这种声明不是装饰，FastAPI 靠它做校验和文档，省掉你手写判断
2. **敏感字段用 `response_model` 兜住**：返回模板和接收模板分开写，密码这类字段自动挡在门外
3. **接口多了早分模块**：别等一个文件塞满几十个路由才想起 `APIRouter`，分货架这事越早越好
